...
use cgmath::{Matrix, Matrix4, vec3, perspective, Deg};
...
const imageVertexShaderSource: &str ="#version 300 es
...
const imageFragmentShaderSource: &str = "#version 300 es
...
impl Sprite {
    pub fn new(gl:&gl::Gl,x:f32,y:f32,image_file:&str,width:f32,height:f32,texture_ref:Option<u32>) -> Sprite {
        let shader_program = create_shader(&gl, imageVertexShaderSource, imageFragmentShaderSource);
    ...
    pub fn render(&mut self, gl:&gl::Gl) {
        ...
        unsafe {
            ...
            self.gl_matrix4(gl, self.transform, "transform");
            self.gl_matrix4(gl, view, "view");
            self.gl_matrix4(gl, projection, "projection");

            gl.DrawElements(gl::TRIANGLES, 6, gl::UNSIGNED_INT, ptr::null());
        }
    }

    fn gl_matrix4(&mut self, gl: &gl::Gl, mat4:Matrix4<f32>, name:&str) {
        unsafe {
            let location = gl.GetUniformLocation(self.shader_program, CString::new(name).unwrap().as_ptr());
            gl.UniformMatrix4fv(
                location,
                1,
                gl::FALSE,
